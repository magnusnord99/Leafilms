...
export function ProjectsBoardView({ view, onViewChange }: {
  view: ProjectsView
  ...
  // Flere pitch-versjoner av samme prosjekt (parent_project_id) vises som ett kort —
  // representativ versjon = den som har kommet lengst i pipelinen. Samme regel som
  // listevisningen bruker, se lib/project-grouping.ts.
  const boardProjects = useMemo(() => {
    const byRoot = groupByProjectRoot(projects)
    return Array.from(byRoot.values()).map(pickRepresentativeVersion)
  }, [projects])
  ...
  const projectsByStage = PIPELINE_STAGES.reduce<Record<string, ProjectWithPipeline[]>>(
    (acc, stage) => {
      acc[stage.value] = boardProjects.filter(p => (p.pipeline_stage ?? 'lead') === stage.value)
      return acc
    },
    {}
  )
  ...
  return (
    <div style={{ background: C.bg, color: C.text, minHeight: '100vh' }}>
      ...
      <div style={{ padding: '24px 28px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '1.4rem', fontWeight: 600, color: C.text, lineHeight: 1.2, marginBottom: 2 }}>Prosjekter</h1>
          <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.75rem', color: C.text3 }}>
            {boardProjects.length} prosjekt{boardProjects.length !== 1 ? 'er' : ''} · dra for å flytte mellom steg · tildel oppgaver direkte på kortene
          </p>
        </div>
