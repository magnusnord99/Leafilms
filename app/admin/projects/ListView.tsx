...
import { STAGE_LABEL, StageGroup, STAGE_GROUP, GROUP_COLOR, GROUP_FILTERS, getStageHref } from '@/lib/pipeline-ui'
import { groupByProjectRoot, pickRepresentativeVersion, STAGE_INDEX } from '@/lib/project-grouping'
...
export function ProjectsListView({ view, onViewChange }: {
  view: ProjectsView
  ...
  // kommet lengst i pipelinen (nyeste ved likt steg) — se lib/project-grouping.ts.
  const cards = useMemo<ProjectCard[]>(() => {
    ...
    return Array.from(byRoot.entries()).map(([rootId, versions]) => {
      const rep = pickRepresentativeVersion(versions)
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(310px, 1fr))', gap: 14 }}>
            {filtered.map(card => {
              ...
              return (
                <div
                  key={card.rootId}
                  onClick={() => router.push(getStageHref(card.overviewId, card.stage))}
                  style={{
