...
export default function ProjectQuotePage({ params }: Props) {
    ...
    try {
      ...
      if (existingQuoteId) {
        ...
        const { data: newQuote, error } = await supabase
          ...
          .single()
        if (error) throw error
        if (!newQuote) throw new Error('Kunne ikke opprette tilbud')
  ...
  async function handleGeneratePDF(data: QuoteBuilderData) {
    ...
    try {
      const quoteId = await handleSave(data)
      if (!quoteId) throw new Error('Kunne ikke lagre tilbud')
