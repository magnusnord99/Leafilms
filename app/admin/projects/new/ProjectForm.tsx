...
export default function ProjectForm({ entry = 'project' }: { entry?: 'project' | 'pitch' }) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const backHref = entry === 'pitch' ? '/admin/pitches' : '/admin'
  const newFormHref = entry === 'pitch' ? '/admin/pitches/new' : '/admin/projects/new'
  ...
  // Hvilket prosjekt et evt. pitch-innhold skal genereres for og lagres på — enten fra
  // ?project_id= (fra prosjektsidens "Opprett med AI"-steg) eller valgt i prosjekt-velgeren
  // under (fra "Ny pitch" → "Eksisterende prosjekt").
  const [reuseProjectId, setReuseProjectId] = useState<string | null>(searchParams.get('project_id'))

  // Steg 0 for entry="pitch": har brukeren valgt om pitchen er for et nytt eller et
  // eksisterende prosjekt? Hoppes over når vi allerede vet det (project_id i URL).
  const [pitchTarget, setPitchTarget] = useState<'new' | 'existing' | null>(
    entry === 'pitch' && !searchParams.get('project_id') ? null : 'new'
  )
  ...
  const [formData, setFormData] = useState({
    ...
    create_pitch: entry === 'pitch' || !!searchParams.get('project_id'),
    delivery_video: '',
    delivery_photo: '',
  ...
  useEffect(() => {
    // Gjenbruk av eksisterende prosjekt (se handleSubmit sin reuseProjectId-gren):
  ...
  // Prosjekter som ennå ikke har en pitch (ingen sections) — de eneste som er trygge å
  // knytte "Ny pitch" til, siden AI-genereringen under overskriver eksisterende
  // seksjonsinnhold uten varsel.
  async function fetchProjectsWithoutPitch() {
    setExistingProjectsLoading(true)
    try {
      const [{ data: allProjects }, { data: sectionRows }] = await Promise.all([
        supabase
          .from('projects')
          .select('id, title, client_name, customer_id, status, updated_at')
          .neq('status', 'archived')
          .order('updated_at', { ascending: false }),
        supabase.from('sections').select('project_id'),
      ])
      const withPitch = new Set((sectionRows ?? []).map(r => r.project_id as string))
      setExistingProjects(((allProjects ?? []) as ExistingProject[]).filter(p => !withPitch.has(p.id)))
    } finally {
      setExistingProjectsLoading(false)
    }
  }

  function selectExistingProject(project: ExistingProject) {
    setReuseProjectId(project.id)
    setFormData(prev => ({ ...prev, title: project.title }))
    if (project.customer_id) {
      setSelectedCustomerId(project.customer_id)
    } else {
      setSelectedCustomerId(null)
      setCustomerInput(project.client_name || '')
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fra prosjekt-huben eller prosjekt-velgeren: gjenbruk eksisterende prosjekt i stedet
      // for å opprette et duplikat (pipeline_stage, status og slug beholdes urørt)
      const existingProjectId = reuseProjectId
  ...
  const showChooser = entry === 'pitch' && pitchTarget === null
  const showExistingPicker = entry === 'pitch' && pitchTarget === 'existing' && !reuseProjectId
  const showForm = !showChooser && !showExistingPicker

  const filteredExistingProjects = (existingProjects ?? []).filter(p => {
    if (!existingProjectSearch.trim()) return true
    const q = existingProjectSearch.toLowerCase()
      ...
      <div className="min-h-screen flex items-center justify-center p-8" style={{ background: C.bg, color: C.text }}>
        <div className="max-w-md w-full text-center">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              type="button"
              onClick={() => window.location.href = newFormHref}
              style={{
                padding: '11px 20px',
                background: 'transparent',
                color: C.text3,
            ...
            >
              {entry === 'pitch' ? 'Lag ny pitch' : 'Opprett nytt prosjekt'}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen p-8 md:p-12" style={{ background: C.bg, color: C.text }}>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <span style={{
              ...
              {entry === 'pitch' ? 'Ny pitch' : 'Nytt prosjekt'}
            </span>
          </div>
          <h1 style={{
            fontFamily: 'var(--font-cormorant)',
            ...
            {entry === 'pitch' ? 'Opprett pitch' : 'Opprett prosjekt'}
          </h1>
          <p style={{
            fontFamily: 'var(--font-dm-sans)',
            fontSize: '0.75rem',
            ...
            {showChooser
              ? 'Skal pitchen knyttes til et eksisterende prosjekt, eller lager du et nytt?'
              : 'Fyll inn informasjon — AI genererer et utkast'}
          </p>
        </div>

        {/* Steg 0 (kun "Ny pitch"): nytt eller eksisterende prosjekt */}
        {showChooser && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              type="button"
              onClick={() => setPitchTarget('new')}
              style={{
                textAlign: 'left',
                padding: '20px 22px',
                background: C.surface,
            ...
            >
              <div style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.8rem', fontWeight: 600, color: C.text, marginBottom: 6 }}>
                Nytt prosjekt
              </div>
              <div style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.68rem', color: C.text3, lineHeight: 1.5 }}>
                Oppretter et helt nytt prosjekt sammen med pitchen
              </div>
            </button>
            <button
              type="button"
              onClick={() => {
                setPitchTarget('existing')
                if (!existingProjects) fetchProjectsWithoutPitch()
              }}
              style={{
                textAlign: 'left',
                padding: '20px 22px',
            ...
            >
              ...
              <div style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.68rem', color: C.text3, lineHeight: 1.5 }}>
                Legg pitchen til et prosjekt som allerede finnes
              </div>
            </button>
          </div>
        )}

        {/* Steg 0b: velg hvilket eksisterende prosjekt */}
        {showExistingPicker && (
          <div>
            <button
              type="button"
              ...
              style={{
                ...
                fontFamily: 'var(--font-dm-sans)', fontSize: '0.6rem', letterSpacing: '0.1em',
                textTransform: 'uppercase', color: C.text3, background: 'none', border: 'none', cursor: 'pointer', padding: 0,
              }}
            >
              ‹ Tilbake til valg
            </button>
            <input
              ...
              onChange={(e) => setExistingProjectSearch(e.target.value)}
              placeholder="Søk på tittel eller kunde..."
              style={{ ...inputStyle, marginBottom: 12 }}
            />
            {existingProjectsLoading && (
              <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.7rem', color: C.text3 }}>Laster prosjekter...</p>
            )}
            {!existingProjectsLoading && filteredExistingProjects.length === 0 && (
              <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.7rem', color: C.text3 }}>
                Fant ingen prosjekter uten pitch fra før.
              </p>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 420, overflowY: 'auto' }}>
              {filteredExistingProjects.map(p => (
                <button
                  key={p.id}
                  type="button"
                ...
                >
                  <span>
                    <span style={{ display: 'block', fontFamily: 'var(--font-dm-sans)', fontSize: '0.75rem', color: C.text }}>
                      {p.title || '(uten tittel)'}
                    </span>
                    <span style={{ display: 'block', fontFamily: 'var(--font-dm-sans)', fontSize: '0.65rem', color: C.text3, marginTop: 2 }}>
                      {p.client_name || 'Ingen kunde'}
                    </span>
                  </span>
                  <span style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.6rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: C.text3, flexShrink: 0 }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          {reuseProjectId && entry === 'pitch' && (
            <button
              type="button"
              onClick={() => { setReuseProjectId(null); setPitchTarget(null); setFormData(prev => ({ ...prev, title: '' })); setSelectedCustomerId(null); setCustomerInput('') }}
              style={{
