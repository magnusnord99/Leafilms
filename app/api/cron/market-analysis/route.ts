...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (updateError) {
      console.error('Cron: analysis completed but result update failed', updateError)
      return Response.json({ error: 'Could not save analysis results' }, { status: 500 })
    }
    ...
    if (updateError) {
      console.error('Cron: could not persist analysis failure', updateError)
    }
