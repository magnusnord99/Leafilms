...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (updateError) {
        console.error('POST /market-analysis result update failed:', updateError)
        return Response.json({ error: 'Analyse fullført, men kunne ikke lagre resultat' }, { status: 500 })
      }
      ...
      if (updateError) {
        console.error('POST /market-analysis error update failed:', updateError)
      }
