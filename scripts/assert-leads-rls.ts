...
const forward = read('supabase/migrations/154_harden_leads_rls.sql')
...
for (const name of policies) {
  assert(
    ...
    `154 must drop existing policy ${JSON.stringify(name)} before recreating it`
  )
  assertIsStaff(policyBlock(forward, name, '154_harden_leads_rls.sql'), name, '154_harden_leads_rls.sql')
}
