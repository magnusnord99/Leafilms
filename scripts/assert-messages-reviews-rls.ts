...
const forward = read('supabase/migrations/156_harden_messages_reviews_rls.sql')
...
for (const name of allForwardPolicies) {
  assert(
    ...
    `156 must drop existing policy ${JSON.stringify(name)} before recreating it`
  )
  assertIsStaff(policyBlock(forward, name, '156_harden_messages_reviews_rls.sql'), name, '156_harden_messages_reviews_rls.sql')
}

assertSelfInsert(
  policyBlock(forward, 'Users can insert own messages', '156_harden_messages_reviews_rls.sql'),
  'user_id',
  'Users can insert own messages',
  '156_harden_messages_reviews_rls.sql'
)
assertSelfInsert(
  policyBlock(forward, 'authed_insert_quote_messages', '156_harden_messages_reviews_rls.sql'),
  'user_id',
  'authed_insert_quote_messages',
  '156_harden_messages_reviews_rls.sql'
)
assertSelfInsert(
  policyBlock(forward, 'authed_insert_reviews', '156_harden_messages_reviews_rls.sql'),
  'requested_by',
  'authed_insert_reviews',
  '156_harden_messages_reviews_rls.sql'
)
assertSelfInsert(
  policyBlock(forward, 'authed_update_reviews', '156_harden_messages_reviews_rls.sql'),
  'reviewer_id',
  'authed_update_reviews',
  '156_harden_messages_reviews_rls.sql'
)
...
assert(
  ...
  'project messages API may keep the service client after the staff gate so chat still works before 156 is applied'
)
assert(
