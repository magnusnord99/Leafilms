...
const forward = read('supabase/migrations/155_harden_pricing_rls.sql')
...
for (const name of allPolicies) {
  assert(
    ...
    `155 must drop existing policy ${JSON.stringify(name)} before recreating it`
  )
  assertIsStaff(policyBlock(forward, name, '155_harden_pricing_rls.sql'), name, '155_harden_pricing_rls.sql')
}
