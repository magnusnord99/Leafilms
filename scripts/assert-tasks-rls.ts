...
const forward = read('supabase/migrations/153_harden_tasks_rls.sql')
...
for (const name of allForwardPolicies) {
  assert(
    ...
    `153 must drop existing policy ${JSON.stringify(name)} before recreating it`
  )
  assertIsStaff(policyBlock(forward, name, '153_harden_tasks_rls.sql'), name, '153_harden_tasks_rls.sql')
}
